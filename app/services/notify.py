    ...
    for r in d["top"]:
        ...
        <table role="presentation" style="width:100%;border-collapse:separate;border:1px solid #e5e7eb;border-radius:12px;margin:0 0 16px;background:#ffffff">
          <tr><td style="padding:16px">
            ...
            {("<div style='font-size:13px;font-weight:600;margin:12px 0 2px'>Cosa dicono davvero (citazioni)</div><ul style='margin:0;padding-left:18px;font-size:12px;color:#374151'>" + "".join(f"<li style='margin:3px 0'>&ldquo;{escape(q.get('quote','')[:160])}&rdquo; <a href='{escape(q.get('url') or '')}' style='color:#1d4ed8'>{escape(q.get('source',''))}</a></li>" for q in (c.get('evidence_quotes') or [])[:3]) + "</ul>") if c.get('evidence_quotes') else ""}
            {("<div style='font-size:12px;color:#6b7280;margin-top:6px'>Trasversale: " + escape(", ".join(f"{v} ({d.get('authors')} pers.)" for v, d in (c.get('evidence_by_vertical') or {}).items())) + "</div>") if c.get('is_cross_vertical') else ""}
...
<div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:600px;margin:0 auto;padding:0 12px;color:#111827">
  ...
  {("<h2 style='margin:24px 0 10px;font-size:15px;color:#374151'>Liquidazioni giudiziali recenti (affitto d'azienda dal curatore)</h2>" + "".join(f"<div style='border:1px solid #e5e7eb;border-radius:10px;padding:12px;margin:0 0 10px;background:#fff'><b>{escape(a.get('name') or '')}</b> <span style='font-size:12px;color:#6b7280'>· {escape(a.get('tribunal') or '')} {a.get('number')}/{a.get('year')} · sentenza {str(a.get('declared_at'))[:10]} · score {a.get('score')}</span><div style='font-size:13px;color:#374151;margin-top:4px'>{escape(a.get('activity') or 'attività da verificare')}</div><div style='font-size:12px;color:#6b7280;margin-top:4px'>curatore: {escape(a.get('curatore') or '')} · {escape('; '.join(a.get('score_reasons') or [])[:150])}</div></div>" for a in d.get('procedures') or [])) if d.get('procedures') else ""}
  {("<h2 style='margin:24px 0 10px;font-size:15px;color:#374151'>Aziende in difficoltà (decreti CIGS, Ministero del Lavoro)</h2>" + "".join(f"<div style='border:1px solid #e5e7eb;border-radius:10px;padding:12px;margin:0 0 10px;background:#fff'><b>{escape(a.get('name') or '')}</b> <span style='font-size:12px;color:#6b7280'>· {escape(a.get('hq') or '')} ({escape(a.get('province') or '')}) · score {a.get('score')}</span><div style='font-size:13px;color:#374151;margin-top:4px'>{escape(a.get('sector') or '')}</div><div style='font-size:12px;color:#6b7280;margin-top:4px'>{a.get('n_decrees')} decreti dal {str(a.get('first_seen'))[:10]} · {escape('; '.join(a.get('score_reasons') or [])[:160])}{(' · <b>' + escape(a.get('news_stage'))) + '</b>' if a.get('news_stage') else ''}</div></div>" for a in d.get('distressed') or [])) if d.get('distressed') else ""}
