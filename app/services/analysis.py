...
def _validate_lenient(schema: type[BaseModel], data: Any) -> dict:
    ...
    if isinstance(data, list) and data and isinstance(data[0], dict) and not any(
            getattr(f.annotation, "__origin__", None) is list for f in schema.model_fields.values()):
        data = data[0]  # small models wrap the object in a list (sometimes one object per input text: keep the first)
    try:
